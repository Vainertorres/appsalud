...
def createNewPac(request):
	...
	pacientes = Paciente.objects.filter(tipodoc_id=tipodocid).filter(identificacion=ident).all()
	#Paciente.objects.filter(estado=True)
...
def enviarsms(celular, mensaje):
	...
	account_sid = os.environ.get('DJANGO_TWILIO_ACCOUNT_SID','secret')
	# Your Auth Token from twilio.com/console
	auth_token  = os.environ.get('DJANGO_TWILIO_AUTH_TOKEN','secret')
